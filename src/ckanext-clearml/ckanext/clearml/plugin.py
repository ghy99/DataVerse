...
def getDataset():
    ...
    try:
        ...
        logging.warning(f"COULD NOT GET DATASET: ERROR: {e}")
...
def getAllDatasetID(dataset_id):
    try:
        ...
        logging.warning(f"FAILED TO RETRIEVE DATASET {dataset_id}, ERROR: {e}")
    return dataset
...
def sortDataset(dataset_IDs, dataset_details):
    ...
    # logging.warning(f"Version list: {versions}, Sorted Version List: {sorted(versions)}")
...
def retrieveDatasetDetails(dataset_details):
    ...
    for key, val in dataset_details.items():
        logging.warning(f"")
        logging.warning(f"")
        logging.warning(f"values: {val.__dict__}")
        logging.warning(f"")
        logging.warning(f"")
...
def renderVersionTree():
    ...
    logging.warning(f"CHECKING DATASET ID AND DETAILS:")
    logging.warning(f"{dataset_IDs}")
